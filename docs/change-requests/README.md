...
| [CR-010](CR-010-element-factories.md) | Generated element factories per module (`<Module>.factory.mjs` + `.d.mts`): creators with `TYPE_NAME`, `{ name, value }` wrappers named by XJC's rule (`createRT`, `createBodyP`), and an `el` map per QName; docx4j's `ObjectFactory` for the TypeScript model | 9 | Implemented 2026-09-10 |
