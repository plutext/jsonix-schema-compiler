...
public class ModuleCompiler<T, C extends T> {
	...
	/**
	 * The module as an ES module: one {@code export const <mapping> = {...};}
	 * per non-empty mapping (CR-005, phase 2). Dependencies need no imports;
	 * Jsonix resolves them by name in {@code new Jsonix.Context([...])}.
	 */
	public String compileEsm() {
		final StringWriter stringWriter = new StringWriter();
		stringWriter.append("// Generated by jsonix-schema-compiler: Jsonix mappings of the module [")
				.append(this.moduleName).append("] as an ES module. Do not edit.\n");
		try {
			for (Mapping<T, C> mapping : this.module.getMappings()) {
				if (!mapping.isEmpty()) {
					...
					stringWriter.append("export const ").append(mappingName).append(" = ");
					new CodeWriter(stringWriter).expression(mappingBody);
					stringWriter.append(";\n");
				}
			}
