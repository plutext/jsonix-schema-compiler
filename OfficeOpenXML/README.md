...
repository, [plutext/docx4j-ts](https://github.com/plutext/docx4j-ts), published as
`@docx4j/docx4j-ts` (CR-009). They were split out of this directory with their history on
...
`docx4j-ts` is empty. After a compiler change, run the script, then `npm run build && npm test`
in `docx4j-ts`, and commit there citing the compiler and docx4j commits.
