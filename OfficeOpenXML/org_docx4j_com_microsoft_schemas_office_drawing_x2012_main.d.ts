// Generated by jsonix-schema-compiler: TypeScript declarations for the Jsonix module [org_docx4j_com_microsoft_schemas_office_drawing_x2012_main]. Do not edit.
// Describes the objects Jsonix produces when unmarshalling with this module's mappings.
import type * as Dep_org_docx4j_dml from './org_docx4j_dml';

/** Jsonix.XML.QName */
export interface XmlQName { namespaceURI: string; localPart: string; prefix?: string; key?: string; string?: string; }
/** Jsonix.XML.Calendar: the representation of date, time and dateTime values; unset fields are NaN. */
export interface XmlCalendar { year?: number; month?: number; day?: number; hour?: number; minute?: number; second?: number; fractionalSecond?: number; timezone?: number; }
/** The representation of xs:duration values. */
export interface XmlDuration { sign?: number; years?: number; months?: number; days?: number; hours?: number; minutes?: number; seconds?: number; }
/** An element as returned by an unmarshaller or held by an elementRef property. */
export interface TypedNamedValue<T> { name: XmlQName; value: T; }
/** A DOM node, allowed where the mapping permits DOM content (allowDom). */
export type XmlDomNode = any;
/** The value of an anyElement property: a typed element, a DOM node or (if mixed) text. */
export type XmlAnyElement = any;
/** The value of an anyAttribute property. */
export interface XmlAnyAttributes { [attributeName: string]: string; }
/** A Jsonix mapping object as consumed by new Jsonix.Context([...]); R is the union of its global element types. */
export interface JsonixMapping<R = unknown> { readonly __rootElement?: R; readonly [key: string]: unknown; }

export interface CTBackgroundPr {
  TYPE_NAME?: 'org_docx4j_com_microsoft_schemas_office_drawing_x2012_main.CTBackgroundPr';
  bwMode?: Dep_org_docx4j_dml.STBlackWhiteMode;
  bwPure?: Dep_org_docx4j_dml.STBlackWhiteMode;
  bwNormal?: Dep_org_docx4j_dml.STBlackWhiteMode;
  targetScreenSize?: string;
}

export interface CTNonVisualGroupProps {
  TYPE_NAME?: 'org_docx4j_com_microsoft_schemas_office_drawing_x2012_main.CTNonVisualGroupProps';
  isLegacyGroup?: boolean;
}

export interface CTObjectPr {
  TYPE_NAME?: 'org_docx4j_com_microsoft_schemas_office_drawing_x2012_main.CTObjectPr';
  objectId?: string;
  isActiveX?: boolean;
  linkType?: string;
}

export interface CTSignatureLine {
  TYPE_NAME?: 'org_docx4j_com_microsoft_schemas_office_drawing_x2012_main.CTSignatureLine';
  isSignatureLine?: boolean;
  id?: string;
  provId?: string;
  signingInstructionsSet?: boolean;
  allowComments?: boolean;
  showSignDate?: boolean;
  suggestedSigner?: string;
  suggestedSigner2?: string;
  suggestedSignerEmail?: string;
  signingInstructions?: string;
  addlXml?: string;
  sigProvUrl?: string;
}

export type BackgroundPrElement = TypedNamedValue<CTBackgroundPr>;

export type NonVisualGroupPropsElement = TypedNamedValue<CTNonVisualGroupProps>;

export type ObjectPrElement = TypedNamedValue<CTObjectPr>;

export type SignatureLineElement = TypedNamedValue<CTSignatureLine>;

/** Any global element of this mapping, i.e. what an unmarshaller returns. */
export type RootElement = BackgroundPrElement | NonVisualGroupPropsElement | ObjectPrElement | SignatureLineElement;

/** The Jsonix mapping [org_docx4j_com_microsoft_schemas_office_drawing_x2012_main] exported by the module's JavaScript file. */
export declare const org_docx4j_com_microsoft_schemas_office_drawing_x2012_main: JsonixMapping<RootElement>;
