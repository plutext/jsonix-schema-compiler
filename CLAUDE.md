...
java -jar full/target/jsonix-schema-compiler-full-<version>.jar \
  [-compact] [-generateJsonSchema] [-generateTypeScript] [-generateFactories] [-logLevel TRACE] [-d outdir] schema.xsd [-b bindings.xjb]
```
...
- `full/` — shaded executable jar bundling XJC, the JAXB 4 runtime, the Jakarta APIs, slf4j-simple and the jaxb-tools XJC plugins (`-Xinheritance`, `-Xannotate`, `-XtoString`, ... plus XJC's own `-Xinject-code`), so schemas carrying those customizations (docx4j's) compile from the command line. `OfficeOpenXML/` holds only the bindings and `generate.sh` for the docx4j mappings; the generated files live in the sibling repository `../docx4j-generated-objects-ts` (`@docx4j/generated-objects-ts`, CR-009). Adds `JsonixMain` (CLI entry point) and the `TargetDirectory*Writer` classes that write files to disk.
...
8. **Element factories** (`compilation.typescript.FactoryCompiler`, CR-010), enabled by `-generateFactories` (implies TypeScript) or `jsonix:typeScript factories="true"`: per module `<base>.factory.mjs` + `.d.mts` (creators `createP(init?)` with `TYPE_NAME` set, one per non-abstract class; element wrappers, one per element declaration: scoped `createRT(value)` by XJC's rule, global `createSettingsElement(value)`) and `<base>.el.mjs` + `.d.mts` (per-local-name wrappers for the module's own namespace, `import * as el`). The `.d.mts` imports the module's declarations as `M`; `TypeScriptModuleCompiler.setOwnQualifier` makes `ref()` qualify own types accordingly. Generated names are never renamed on collision: the build fails naming both sources. Golden files: `compiler/src/test/resources/typescript/po/PurchaseOrder.{factory,el}.*`.
...
- Required/optional and `minOccurs`/`maxOccurs` come from `xml.xsom.EffectiveMultiplicity` (the property's particle within its class's content model: choice branches and optional groups make it optional), used by both the mapping and TypeScript emitters, with `ParticleMultiplicityCounter` as fallback.
